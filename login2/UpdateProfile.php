...
require_once("resetGameID.php");
  ...
  if(!(isset($_SESSION['loggedin']))){
    ...
    if(empty(trim($_POST["user"]))){
        ...
        $sql_u = "SELECT * FROM Users WHERE USERNAME = '$uusername' ";
    ...
    if(empty(trim($_POST["email"]))){
        ...
        // Prepare a select statement
       		 $sql_e = "SELECT * FROM Users WHERE EMAIL = '$eemail' ";
	...
	$sqlq = "UPDATE Users SET USERNAME='$new_username', FIRSTNAME='$new_firstname' , LASTNAME='$new_lastname', EMAIL = '$new_email' WHERE ID = '$id'";
...
<div class=container-for-all style="background-color: #e5e5c7;">
	...
	<div class="fs">
		...
		<div class="<?php echo (!empty($role)) ? 'has-error' : ''; ?>">
			<label for="role"><b>Role</b></label>
			<br>
			<input type="text" value="<?php echo $role; ?>" name="role" readonly>
		</div>
	</div>
	...
	<div class="fs">
		...
		<div class="<?php echo (!empty($mobile_phone_err)) ? 'has-error' : ''; ?>">
			<label for="mobile_numb"><b>created_at</b></label>
			<br>
			<input type="text" value="<?php echo $created_date; ?>" name="mobl" readonly>
		</div>
