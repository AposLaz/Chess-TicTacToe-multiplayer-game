...
/*
	Elegxoume to apotelesma gia nikiti
*/
function CheckResult() {
	...
	if(InCheck == BOOL.TRUE)	{
	    if(brd_side == COLOURS.WHITE) { //kerdisan ta maura
			...
			return BOOL.TRUE;
	     // $("#GameStatus").text(data.msg);return BOOL.TRUE;
        } else {		//kerdisan ta leuka
	        var id1 = $("#playerID").html();	 
				//console.log(id1);
				$.ajax({
					type: "POST",
					url: "winner.php",
...
/* Gia na paroume to xrwma tou xristi kaloume mia sunartisi pou exoume sto /pages/ch/....
	Oi times opws fainetai parakatw tha einai 0 i 1
*/
...
/* 
	Analoga me to xrwma tou xristi to board ginete katallila rithmizomeno
*/
flipGameOnceIfBlack();
...
/* Arxika elegxoume to xrwma tou xristi apo mia sunartisi parapanw pou tha einai 
	0 = white
	1 = black
 */ 
function MakeUserMove2() {
	...
	/*An patisoume se kapoio pioni tote energopoieitai to if 
	perimenontas na kanoume kapoia kinisi */
	if(UserMove.from != SQUARES.NO_SQ && UserMove.to != SQUARES.NO_SQ) {
		...
		/* To if parakatw energopoieitai na patisoume na kanoume ti kinisi,opote
		   kanei kai tous sxetikous elegxous gia to xrwma kai to board.
		   Autoi ginete giati,an eimai o white tote den prepei na mporw na 
		   metakinisw to mauro kai antistrofa.
		*/
		...
		/*
			Otan kanoume mia kinisi tote prepei na apothikeuoume ti kinisi sti DB.
			Autin i kinisi emfanisetai se kathe paixti kai prepei na kathe fora 
			pou ginete mia ne elegxoume gia mia nea kinisi.
		*/
		var MoveString = BoardToFen();
				$.ajax({
					type:'POST',
					url:'../InsertMoveTournament.php',
					data:{MoveString:MoveString},
					dataType:'json',
					success:function(data){
						//alert(data.msg);
