...
//////////////// check Opponent Exist
		public function checkOpponentExist($oppID){
			...
			//get gameID
			$sql = "SELECT * FROM GameTic WHERE UserID = '$oppID' ";
...
////////////////You left You lost yr opponent Win 
		public function getWinOpponentLeft($myID,$oppID){
			...
			$Finish = 1; //kerdizei
			$sql = "UPDATE GameTic SET Finish = '$Finish' WHERE UserID = '$oppID' ";
...
///////////////finish reset
		public function finishReset($myID){
			...
			$val = 0;
			//update prwta emas
    		$sql = "UPDATE GameTic SET Finish = '$val' WHERE UserID = '$myID' ";
...
//////////////begin NEW game
		public function getNewGame($myID,$oppID){
			...
			//get gameID
			$sql = "SELECT * FROM GameTic WHERE UserID = '$myID' ";
			$result = mysqli_query($link, $sql) or die("Bad connection.Try later");
			$row = mysqli_fetch_assoc($result);
			$gameID = $row['GameID'];
			...
			if($posit == 1){
	       	...
	       	//update prwta emas
    		$sql = "UPDATE GameTic SET Color = '$color' , Turn = '$myturn' , Finish = '$val' WHERE UserID = '$myID' ";
            $result3 = mysqli_query($link, $sql) or die(mysqli_error($link));
            
            //epeita kanoume update ton antiplao mas
            $sql = "UPDATE GameTic SET Color = '$color_opp' , Turn = '$oppturn' , Finish = '$val' WHERE UserID = '$oppID' ";
...
//////////////draw function
		public function getDraw($myID,$oppID)
		{
			...
			//-----------egw
				...
				$sql = "UPDATE ScoresTic SET Pdraw = '$draw' WHERE  User_id = '$myID' ";
				$result2 = mysqli_query($link, $sql) or die("Bad connection.Try later");

				$Finish = 3; //isopalia
				$sql = "UPDATE GameTic SET Finish = '$Finish' WHERE UserID = '$myID' ";
				$result3 = mysqli_query($link, $sql) or die("Bad connection.Try later");
			...
			//----------antipalos
				...
				$sql = "UPDATE ScoresTic SET Pdraw = '$dra' WHERE  User_id = '$oppID' ";
				$result4 = mysqli_query($link, $sql)or die("Bad connection.Try later");
...
//////////////function winnerGreen
		public function getWinnerGreen($myID,$oppID,$mycolor){
			...
			if($mycolor == "green"){//kerdizw
				...
				$sql = "UPDATE ScoresTic SET Pwin = '$win' WHERE  User_id = '$myID' ";
				$result2 = mysqli_query($link, $sql) or die("Bad connection.Try later");
				...
				$sql = "UPDATE ScoresTic SET Plosse = '$losse' WHERE  User_id = '$oppID' ";
				$result4 = mysqli_query($link, $sql) or die("Bad connection.Try later");
				...
				$result = mysqli_query($link, $sql) or die("Bad connection.Try later");
				$row = mysqli_fetch_assoc($result);
				$Plosse = $row['Plosse'] + 1; //xanw

				$sql = "UPDATE ScoresTic SET Plosse = '$Plosse' WHERE  User_id = '$myID' ";
				$result2 = mysqli_query($link, $sql) or die("Bad connection.Try later");
...
/////////////Update My turn
		public function updateTurn($id){
			...
			$sql = "UPDATE GameTic SET Turn = '$turn' WHERE UserID = '$id' ";
           	$result3 = mysqli_query($link, $sql) or die(mysqli_error($link));
...
///////pairnoume to guro tou xristi
		...
		public function UserLogin(){
			...
			$sql = "SELECT * FROM GameTic WHERE UserID = '$userID' ";	
			$result = mysqli_query($link, $sql) or die("Bad connection.Try later");
		...
		public function DisplayAvailablePlayers(){
			...
			$sql = "SELECT * FROM GameTic WHERE GameID <> 0 ORDER BY GameID ";	
			$result = mysqli_query($link, $sql) or die("Bad connection.Try later");
			...
			if (mysqli_num_rows($result) > 1) {
				while($row = mysqli_fetch_assoc($result)){
					...
					if($row["UserID"] != $_SESSION['UserId']){ // if opponent name,den efmanizei to onoma mas
						...
						if ($row["GameID"] != 1 || $user_exist == true){
							...
							if($row["GameID"] > 0){
								...
								<li class="media" style="border:2px solid black;border-radius: 10px;background-color:green;">
							        <div class="row">
							          ...
							          <?php 
							          	$my_exist = $tourn->ExistUser($myIDI);
							          if ($my_exist != true) { 
							          ?>
							          	<div class="col-lg-3">
							          ...
							          <?php }else{ ?> 
							          	<div class="col-lg-3">
							              <p style="color: black;cursor: pointer" id="avatarName" onclick="parent.location='redirect.php?id=<?php echo $token;?>&name=<?php echo $id;?>';">Can't Stay here</p>
							              <img style="cursor: pointer;border:1px solid black;" class="mr-3" id="avatar" src="img/avatar.jpg" alt="Generic placeholder image">
		...
		public function DeleteGame($id){
			...
			$sql = "SELECT * FROM GameTic ORDER BY UserID ";	
			$result = mysqli_query($link, $sql) or die("Bad connection.Try later");
			...
			if (mysqli_num_rows($result) > 0) {
				while($row = mysqli_fetch_assoc($result)){
					...
					if($gameID == $id) {
						...
						$sql = "UPDATE GameTic SET OppID = '$OppID' , GameID = '$GameID' , Color = '$Color' , Turn = '$Turn' , Finish = '$Turn' WHERE UserID = '$UserId' ";
           				 $result3 = mysqli_query($link, $sql) or die(mysqli_error($link));
