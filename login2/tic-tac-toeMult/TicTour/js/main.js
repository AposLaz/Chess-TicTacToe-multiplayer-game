...
setInterval(function(){
	...
	if(finishMsg == 1 && fini == 0) //kerdizw 
	{
		boardMsg("You WIN !!!");
				...
				fini = fini + 1;
				//an kerdisoume kanoume update to round
				var id = $('#playerID').text();
				$.ajax({
					type: "POST",
					url: "main_functions.php",
					data: {UpdateRoundID:id},
					dataType: "json",
					async:false,
					success:function(data){
						//console.log(data.msg);
		...
		boardMsg("You DRAW !!!");
				Swal.fire({
				  title: "DRAW.You will play again!!!",
				  height: 500,
				  confirmButtonColor: 'red',
				  showClass: {
				    popup: 'animated fadeInDown faster'
				...
				$(".swal2-modal").css('border', '1px solid black');
				fini = fini + 1;
				//an erthei tote exoume retach kanoume update to round
				var id = $('#playerID').text();
				var oppID = $('#opponentID').text();
				$.ajax({
					type: "POST",
					url: "main_functions.php",
					data: {DrawTourID:id,oppID:oppID},
					dataType: "json",
					async:false,
					success:function(data){
						console.log(data.msg);
...
setInterval(function(){
	...
	if(checkOppoennt_exist == 1){
		//console.log(checkOppoennt_exist);
